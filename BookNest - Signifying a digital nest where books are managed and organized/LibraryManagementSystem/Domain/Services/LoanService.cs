...
namespace LibraryManagementSystem.Domain.Services
{
    ...
    public class LoanService
    {
        public Loan LoanBook(Book book, Member member, DateTime loanDate)
        {
            ...
            // Check if book is available for loan. If not, throw and InvalidOperationException with the message "The book is not available for loan."
            if (!book.IsAvailable()){
                throw new InvalidOperationException("The book is not available for loan."); 
            }
            // Check if member can borrow. If not, throw an InvalidOperationException with the message "The member has reached their borrowing limit."
            if (!member.CanBorrow()){
                throw new InvalidOperationException("The member has reached their borrowing limit.");
            }
            // Finally call the RecordLoan method on the member object and pass the loan object as an argument
